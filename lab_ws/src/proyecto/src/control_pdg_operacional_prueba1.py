...
qdes = np.array([0., 0., 0., 0., 0., 1.6, 0.0, 0., 0., 0., 0.])
#q = qdes
# =============================================================
...
modelo = rbdl.loadModel(
    '/home/user/lab_ws/src/proyecto/urdf/camara_movil_generated.urdf')
...
while not rospy.is_shutdown():
    ...
    """# Limites de articulaciones de revolucion
    ...
    # Jacobiano analitico con la posicion actual
    J = jacobian_position(q[0: 7])
    # Posicion actual del efector final
    x = fkine(q[0: 7])[0: 3, 3]
    e = xdes-x  # Error de posicion

    # Hallar posicion actual (representado por esfera roja)
    x = fkine(q[0: 7])[0: 3, 3]
    jstate.header.stamp = rospy.Time.now()
    ...
    # Dinamica inversa - calculo del vector de gravedad
    g = np.zeros(ndof+4)
